  ...
  namespace :puma do
    ...
    task :configure_monit, roles: lambda { fetch(:puma_roles) }, on_no_matching_servers: :continue do
      ...
      upload_monit_config(fetch(:puma_service_name), {
        ...
        start_command: "#{fetch :puma_shell} -c 'cd #{current_path} ; $rvm_path/bin/rvm rvmrc trust load ; nohup bundle exec puma -e #{rails_env} -S #{fetch :puma_state_file} --port=#{fetch :puma_port} --pidfile=#{fetch :puma_pid_file} --bind=unix://#{fetch :puma_sock_file} >> #{fetch :puma_log_file} 2>&1 &'",
