  ...
  namespace :puma do
    ...
    task :configure_monit, roles: lambda { fetch(:puma_roles) }, on_no_matching_servers: :continue do
      ...
      upload_monit_config("puma", {
        ...
        start_command: "#{fetch :puma_shell} -c 'cd #{current_path} ; nohup bundle exec puma -e #{rails_env} -S #{fetch :puma_state_file} --port=#{fetch :puma_port} --pidfile=#{fetch :puma_pid_file} >> #{fetch :puma_log_file} 2>&1 &'",
